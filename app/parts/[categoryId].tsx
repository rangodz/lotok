import React from 'react';
import {
  Pressable,
  ScrollView,
  StyleSheet,
  Text,
  View,
} from 'react-native';
import Animated, { FadeInDown } from 'react-native-reanimated';
import { useLocalSearchParams, useRouter } from 'expo-router';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useTranslation } from 'react-i18next';
import {
  AlertTriangle,
  ArrowLeft,
  Copy,
  MapPin,
  ShieldCheck,
  XCircle,
} from 'lucide-react-native';
import { colors, radius, spacing, typography } from '@/lib/theme';
import { oilFilterResult as data } from '@/lib/mock';
import { Button, Card } from '@/components/ui';
import { FitmentIndicator } from '@/components/FitmentIndicator';
import { PartThumbnail } from '@/components/PartThumbnail';
import { ltrText } from '@/lib/rtl';

export default function PartResultScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { t } = useTranslation();
  // categoryId will drive dynamic data fetching in Phase 2
  const { categoryId } = useLocalSearchParams<{ categoryId: string }>();

  return (
    <View style={styles.root}>
      {/* Header */}
      <View style={[styles.header, { paddingTop: insets.top + spacing.sm }]}>
        <Pressable
          onPress={() => router.back()}
          style={styles.backButton}
          accessibilityRole="button"
          accessibilityLabel={t('common.back')}
        >
          <ArrowLeft size={22} color={colors.textPrimary} />
        </Pressable>
        <View>
          <Text style={styles.headerTitle}>{data.category}</Text>
          <Text style={styles.headerSubtitle}>{data.vehicleLabel}</Text>
        </View>
      </View>

      <ScrollView
        contentContainerStyle={{ padding: spacing.md, paddingBottom: 120 }}
        showsVerticalScrollIndicator={false}
      >
        {/* OEM card */}
        <Animated.View entering={FadeInDown.duration(350).springify()} style={styles.oemCard}>
          <View style={styles.oemHeader}>
            <ShieldCheck size={18} color={colors.kartoPale} />
            <Text style={styles.oemLabel}>{t('partResult.oemLabel')}</Text>
          </View>
          <View style={styles.oemRow}>
            <Text style={[styles.oemCode, ltrText]}>{data.oem.code}</Text>
            <Pressable style={styles.copyButton} accessibilityLabel={t('partResult.copyRef')}>
              <Copy size={16} color={colors.white} />
            </Pressable>
          </View>
          <Text style={styles.oemNote}>{data.oem.note}</Text>
        </Animated.View>

        {/* Equivalents */}
        <Text style={styles.sectionTitle}>{t('partResult.equivalences')}</Text>
        <View style={{ gap: spacing.sm }}>
          {data.equivalents.map((part) => (
            <Card key={part.ref} style={styles.partCard}>
              <PartThumbnail categoryId={categoryId ?? ''} partRef={part.ref} size={48} />
              <View style={{ flex: 1 }}>
                <Text style={styles.partName}>
                  {part.manufacturer}{' '}
                  <Text style={[styles.partRef, ltrText]}>{part.ref}</Text>
                </Text>
                <FitmentIndicator level={part.fitmentLevel} />
              </View>
            </Card>
          ))}
        </View>

        {/* Brands to avoid */}
        <Text style={styles.sectionTitle}>{t('partResult.brandsToAvoid')}</Text>
        <Card style={styles.avoidCard}>
          {data.avoid.map((item, i) => (
            <View
              key={item.ref}
              style={[
                styles.avoidRow,
                i > 0 && { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.border },
              ]}
            >
              <XCircle size={18} color={colors.warning} />
              <View style={{ flex: 1 }}>
                <Text style={styles.avoidName}>{item.brand} {item.ref}</Text>
                <Text style={styles.avoidReason}>{item.reason}</Text>
              </View>
            </View>
          ))}
        </Card>

        {/* Counterfeit */}
        <Text style={styles.sectionTitle}>{t('partResult.knownCounterfeit')}</Text>
        <Card style={styles.counterfeitCard}>
          <View style={styles.counterfeitHeader}>
            <AlertTriangle size={20} color={colors.dangerDeep} />
            <Text style={styles.counterfeitTitle}>
              « {data.counterfeit.brand} » — {data.counterfeit.ref}
            </Text>
          </View>
          {data.counterfeit.signs.map((sign) => (
            <View key={sign} style={styles.signRow}>
              <View style={styles.signDot} />
              <Text style={styles.signText}>{sign}</Text>
            </View>
          ))}
          <Text style={styles.counterfeitFooter}>
            {t('partResult.counterfeitReports', { count: 12 })}
          </Text>
        </Card>
      </ScrollView>

      {/* CTA */}
      <View style={[styles.ctaWrap, { paddingBottom: insets.bottom + spacing.sm }]}>
        <Button
          variant="cta"
          title={t('partResult.findNearMe')}
          onPress={() =>
            router.push({
              pathname: '/(tabs)/shops',
              params: { partRef: data.oem.code },
            })
          }
        />
        <View style={styles.ctaHintRow}>
          <MapPin size={14} color={colors.textMuted} />
          <Text style={styles.ctaHint}>{t('partResult.shopsNearby', { count: 3 })}</Text>
        </View>
      </View>
    </View>
  );
}

const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: colors.bg },

  header: {
    backgroundColor: colors.white,
    paddingHorizontal: spacing.md,
    paddingBottom: spacing.sm,
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.sm,
    borderBottomWidth: 1,
    borderBottomColor: colors.border,
  },
  backButton: {
    width: 40,
    height: 40,
    borderRadius: radius.full,
    backgroundColor: colors.surface,
    alignItems: 'center',
    justifyContent: 'center',
  },
  headerTitle: { ...typography.h2, color: colors.textPrimary },
  headerSubtitle: { ...typography.caption, color: colors.textSecondary },

  oemCard: {
    backgroundColor: colors.karto,
    borderRadius: radius.lg,
    padding: spacing.md,
  },
  oemHeader: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  oemLabel: {
    ...typography.tiny,
    color: colors.kartoPale,
    textTransform: 'uppercase',
    letterSpacing: 1,
  },
  oemRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    marginTop: spacing.sm,
  },
  oemCode: {
    fontFamily: 'Sora_700Bold',
    fontSize: 26,
    color: colors.white,
    letterSpacing: 0.5,
  },
  copyButton: {
    width: 36,
    height: 36,
    borderRadius: radius.sm,
    backgroundColor: 'rgba(255,255,255,0.14)',
    alignItems: 'center',
    justifyContent: 'center',
  },
  oemNote: { ...typography.caption, color: colors.kartoNote, marginTop: spacing.sm },

  sectionTitle: {
    ...typography.h3,
    color: colors.textPrimary,
    marginTop: spacing.lg,
    marginBottom: spacing.sm,
  },

  partCard: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm },
  partName: { ...typography.h3, fontSize: 15, color: colors.textPrimary, marginBottom: 4 },
  partRef: { fontFamily: 'Inter_500Medium', color: colors.textSecondary },

  avoidCard: {
    backgroundColor: colors.warningBg,
    padding: 0,
    overflow: 'hidden',
  },
  avoidRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.sm,
    padding: spacing.md,
  },
  avoidName: { ...typography.h3, fontSize: 14, color: colors.warningDeep },
  avoidReason: { ...typography.caption, color: colors.warningDeep },

  counterfeitCard: {
    backgroundColor: colors.dangerBg,
  },
  counterfeitHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.sm,
    marginBottom: spacing.sm,
  },
  counterfeitTitle: {
    ...typography.h3,
    fontSize: 15,
    color: colors.dangerDeep,
    flex: 1,
  },
  signRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, marginTop: 6 },
  signDot: {
    width: 5,
    height: 5,
    borderRadius: 3,
    backgroundColor: colors.dangerDeep,
    marginTop: 7,
  },
  signText: { ...typography.body, color: colors.dangerDeep, flex: 1 },
  counterfeitFooter: {
    ...typography.caption,
    color: colors.dangerDeep,
    marginTop: spacing.sm,
    fontFamily: 'Inter_600SemiBold',
  },

  ctaWrap: {
    position: 'absolute',
    bottom: 0,
    left: 0,
    right: 0,
    backgroundColor: colors.white,
    paddingHorizontal: spacing.md,
    paddingTop: spacing.sm,
    borderTopWidth: 1,
    borderTopColor: colors.border,
  },
  ctaHintRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    gap: 4,
    marginTop: 8,
  },
  ctaHint: { ...typography.caption, color: colors.textMuted },
});
